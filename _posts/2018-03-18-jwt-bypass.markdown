...
&emsp;&emsp;验证方法：首先服务端会产生一个`key`，然后以这个`key`作为密钥，使用第一部分选择的加密方式（这里就是`HS256`），对第一部分和第二部分`拼接的结果`进行加密，然后把加密结果放到`第三部分`。服务器每次收到信息都会都它的前两部分进行加密，然后比对加密后的结果`是否`跟客户端传送过来的`第三部分相同`，如果相同则验证通过，否则失败。
